-- DATE, TIME INTERVAL, TIMESTAMP (DATE, TIME AND TIME ZONE)

-- 1. DATE AND TIME

-- 1.1 Extracting components of a timestamp using date_part().
SELECT
...
-- 1.2 Use extract() for similar datetime parsing
SELECT extract(year from '2022-12-01 18:37:12 EST'::timestamptz) AS year;

-- 1.3 Retrieve the current date and time
SELECT
    current_timestamp,
    localtimestamp,
    current_date,
...
-- 1.4 Compare current_timestamp and clock_timestamp() on insert
...
-- 2.1 Viewing your current time zone setting
SHOW timezone; -- Note: You can see all run-time defaults with SHOW ALL;
SELECT current_setting('timezone');

-- 2.2 Using current_setting() inside another function
SELECT make_timestamptz(2022, 2, 22, 18, 4, 30.3, current_setting('timezone'));

-- 2.3 Showing time zone abbreviations and names
SELECT * FROM pg_timezone_abbrevs ORDER BY abbrev;
SELECT * FROM pg_timezone_names ORDER BY name;

-- 2.4 Filter to find one
SELECT * FROM pg_timezone_names
WHERE name LIKE 'Europe%'
ORDER BY name;

-- 2.5 Setting the time zone for a client session
...
-- 3.1 Creating a table and importing NYC yellow taxi data
...
-- 3.2 Count the trip records
SELECT count(*) FROM nyc_yellow_taxi_trips;

-- 3.3 Counting taxi trips by hour
